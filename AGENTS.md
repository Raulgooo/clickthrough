...
It observes the current browser context, understands the user's intent, and generates the exact overlay UI needed to verify, understand, act, or respond without making the user leave the page.
...
- **The Copilot That Ships**: render UI for users to confirm, tweak, and execute inline.
...
- **Act**: SharkAuth full-permissions API key action surface.
...
- maps well to skeletons, progress, partial UI trees, action requests, and final UI state
...
But approval gates and sensitive actions must remain visibly Clickthrough-controlled.

## Safety Rules

- Never execute page actions without approval.
- Always show risk for destructive, permission-changing, external, or sensitive actions.
